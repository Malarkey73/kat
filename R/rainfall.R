...
#' @param chr the chromosome (character)
...
#' rainfall(VCF, chr = 6, lower = 1.24e8, upper = 1.4e8, facets="strand", colour="mutation_type")
#'

rainfall <- function(VCF, chr, lower, upper, colour=NULL, facets=NULL, shape=NULL, bursts=F, s=3, gamma=3, vline=NULL, ...){
  library(dplyr)
  library(ggplot2)
  theme_set(theme_bw())
  ...
  kat <- VCF %>%
    filter(chr == chr, start.position > lower, end.position < upper) %>%
